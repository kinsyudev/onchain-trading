...
ponder.on("UniswapV2PairBase:setup", async () => {
...
// Setup hook to initialize RabbitMQ client
ponder.on("UniswapV2PairMainnet:setup", async () => {
  // Initialize RabbitMQ client during setup phase
  await getRabbitMQClient();
...
ponder.on("UniswapV2PairBase:Swap", async ({ event, context }) => {
...
ponder.on("UniswapV2PairMainnet:Swap", async ({ event, context }) => {
  // Get the RabbitMQ client
  const rabbitmqClient = await getRabbitMQClient();
